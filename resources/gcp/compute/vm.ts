...
function genVm(name: string, machineType: string, osImage: string) {
  // Import the program's configuration settings.
  const config = new pulumi.Config();
  const machine_type = machineType || config.get("machineType") || "f1-micro";
  const os_Image = osImage || config.get("osImage") || "debian-11";
